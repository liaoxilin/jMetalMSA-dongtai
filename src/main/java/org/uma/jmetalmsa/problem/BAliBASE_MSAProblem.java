...
public class BAliBASE_MSAProblem extends MSAProblem {
  ...
  public BAliBASE_MSAProblem(String problemName, String dataBaseDirectory, List<Score> scoreList)
    ...
    listOfSequenceNames = readSeqNameFromAlignment(BalibasePath + InstanceBalibase + ".tfa");//存序列名

    originalSequences = readDataFromFastaFile(BalibasePath + InstanceBalibase + ".tfa");//存序列
