...
public class InsertARandomGapMSAMutation implements MutationOperator<MSASolution> {
  ...
  public void doMutation(MSASolution solution) {
    if (randomGenerator.nextDouble() < mutationProbability) {  //如果随机产生的数<变异算子
       
       int sizeOfTheAlignment=solution.getAlignmentLength();  //获得对齐序列长度
  ...
  public List<Integer>  insertARandomGap(List<Integer> gapsGroup, int sizeOfTheAlignment) {  //随机插入一个“-”
    Integer posRandomGap;
    //System.out.println(gapsGroup.toString());
    posRandomGap = randomGenerator.nextInt(0, sizeOfTheAlignment-1);  //随机选取一个位置
