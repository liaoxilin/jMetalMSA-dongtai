...
public abstract class AbstractMOEAD<S extends Solution<?>> implements Algorithm<List<S>> {
    ...
    protected double[] idealPoint;   //理想点的数值，双目标-->二维
    // nadir point

    protected double[][] NeighIdealPoint;
    protected double[][] NeighWorstPoint;

    protected double[] nadirPoint;
    /** Lambda vectors */
    protected double[][] lambda;   //划分空间的坐标向量，二维的从（0，1）--->(1,0)
    /** T in Zhang & Li paper */
    protected int neighborSize;    //固定的20
    protected int[][] neighborhood;  //邻域，每个向量的20个邻居向量
    ...
    public AbstractMOEAD(Problem<S> problem, int populationSize, int resultPopulationSize,
        ...
        NeighIdealPoint = new double[populationSize][problem.getNumberOfObjectives()];
        NeighWorstPoint = new double[populationSize][problem.getNumberOfObjectives()];
    ...
    protected void initializeIdealPoint() {
        ...
        for(int i=0; i<populationSize; i++){
            for (int n = 0; n < problem.getNumberOfObjectives(); n++) {
                ...
                if (population.get(i).getObjective(n) < idealPoint[n]) {
                    idealPoint[n] = population.get(i).getObjective(n);
                }
            }
        }
...
//        }
    ...
    //12.6
    protected void initializeNeighIdealPoint() {
        for(int i = 0;i<populationSize;i++){
            for(int j=0;j<problem.getNumberOfObjectives();j++){
                NeighIdealPoint[i][j] = 1.0e+30;
            }
        }
        for(int i = 0; i<populationSize; i++){
            for(int j=0; j<neighborSize; j++){
                for(int k =0; k<problem.getNumberOfObjectives(); k++){
                    problem.evaluate(population.get(neighborhood[i][j]));
                    if(population.get(neighborhood[i][j]).getObjective(k) < NeighIdealPoint[i][k]){
                        NeighIdealPoint[i][k] = population.get(neighborhood[i][j]).getObjective(k);
                    }
                }
    ...
    protected void initializeNeighWorstPoint() {
        for(int i = 0;i<populationSize;i++){
            for(int j=0;j<problem.getNumberOfObjectives();j++){
                NeighWorstPoint[i][j] = -Double.MAX_VALUE;
            }
        }
        ...
        for(int i = 0; i<populationSize; i++){
            for(int j=0; j<neighborSize; j++){
                for(int k =0; k<problem.getNumberOfObjectives(); k++){
                    problem.evaluate(population.get(neighborhood[i][j]));
//                    System.out.println(population.get(neighborhood[i][j]).getObjective(k));
//                    System.out.println(NeighWorstPoint[i][k]);
                    if(population.get(neighborhood[i][j]).getObjective(k) > NeighWorstPoint[i][k]){
                        NeighWorstPoint[i][k] = population.get(neighborhood[i][j]).getObjective(k);
                    }
                }
            }
        }
    ...
    //12.6
    protected void updateNeighIdealPoint(S individual, int subProblemId) {
        for (int n = 0; n < problem.getNumberOfObjectives(); n++) {
            if (individual.getObjective(n) < NeighIdealPoint[subProblemId][n]) {
                NeighIdealPoint[subProblemId][n] = individual.getObjective(n);
            }
        }
    }
    protected void updateNeighWorstPoint(S individual, int subProblemId) {
        for (int n = 0; n < problem.getNumberOfObjectives(); n++) {
            if (individual.getObjective(n) > NeighWorstPoint[subProblemId][n]) {
                NeighWorstPoint[subProblemId][n] = individual.getObjective(n);
            }
        }
    }
    ...
    //12.6 判断当前合适的种群
    protected NeighborType changeNeighborType(S individual, int subProblemId){

            NeighborType neighborType = null;
            for(int i=0; i<individual.getNumberOfObjectives(); i++){
                if(individual.getObjective(i)< NeighWorstPoint[subProblemId][i]){
                        neighborType = NeighborType.NEIGHBOR;
                }
                if(individual.getObjective(i)< NeighIdealPoint[subProblemId][i]){
                        neighborType = NeighborType.POPULATION ;
                }
    ...
    protected  void updateNeighborhood(S individual, int subProblemId, NeighborType neighborType) throws JMetalException {
        ...
        MOEADUtils.randomPermutation(perm, size);   //perm是一个随机0——size-1的数组
    ...
    double fitnessFunction(S individual, double[] lambda) throws JMetalException {
        ...
        if (MOEAD.FunctionType.TCHE.equals(functionType)) {
            ...
            //theta = evaluations*10.0/maxEvaluations;
            //theta = Math.pow(10.0,evaluations*1.0/maxEvaluations);
            //theta = 10 * Math.pow(evaluations*1.0/maxEvaluations,2.0);
            //System.out.println(theta + " >>>>> " + evaluations + " >>>>> " + maxEvaluations);
            d1 = d2 = nl = 0.0;
